...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    if (IS_DIGIT(*ptr)) {   //first letter cannot be digit
        // printf("Func: parse_symbol. ERROR!\n");
        return PARSE_ERR;
    }
    ...
    if (parse_instr(&ptr, &instr) != PARSE_ERR) {
        ...
        switch (icode) {    //parse different instr
            ...
            case I_ALU: {
                ...
                // parse_reg(&ptr, &regidA);
                // parse_delim(&ptr, ',');
                // parse_reg(&ptr, &regidB);
            ...
            case I_DIRECTIVE: {
                switch (ifun) {     //parse directive code
                    ...
                    case D_POS: {
                        int64_t val;
                        char *name;
                        parse_t parse_type = parse_data(&ptr, &name, &val);
...
int relocate(void)
{
    ...
    while (rtmp)
    {
        ...
        switch (HIGH(rtmp->y64bin->codes[0])) {
            ...
            case I_JMP:{
                for (int i = 1; i < 9; i++) {
                    rtmp->y64bin->codes[i] = dest >> ((i - 1) * 8);
                }
                break;
            }
            case I_DIRECTIVE:{
                for (int i = 0; i < 8; i++) {
                    rtmp->y64bin->codes[i] = dest >> (i * 8);
                }
                break;
            }
            default:
                for (int i = 2; i < 10; i++) {
                    rtmp->y64bin->codes[i] = dest >> ((i - 2) * 8);
                }
                break;
        }
        /* next */
