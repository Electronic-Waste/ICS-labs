...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    if (op == A_ADD || op == A_SUB) 
        ovf = (argA < 0 == argB < 0) && (val < 0 != argA < 0) && argA != 0 && argB != 0;
...
stat_t nexti(y64sim_t *sim)
{
    ...
    byte_t reg = 0;
    regid_t regA, regB;
    if (icode >= 2 && icode <=6 || icode == 0xA || icode == 0xB) {
    ...
    switch (icode) {
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        if (NORM_REG(regA) && NORM_REG(regB)) {
            ...
            if (!set_long_val(sim->m, valB + imm, valA)) {
                err_print("PC = 0x%lx, Invalid data address 0x%016lx", sim->pc, valB + imm);
                return STAT_ADR;
            }
        }
        else {
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        if (NORM_REG(regA) && NORM_REG(regB)) {
            long_t val;
            long_t valB = get_reg_val(sim->r, regB);
            if (!get_long_val(sim->m, valB + imm, &val)) {
                err_print("PC = 0x%lx, Invalid data address 0x%016lx", sim->pc, valB + imm);
                return STAT_ADR;
            }               
      ...
      case I_JMP: /* 7:x imm */
        ...
        break;    
      case I_CALL: /* 8:x imm */ {
        long_t valOfRsp = get_reg_val(sim->r, REG_RSP);
        set_reg_val(sim->r, REG_RSP, valOfRsp - 8);
        if (!set_long_val(sim->m, valOfRsp - 8, next_pc)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%016lx", sim->pc, valOfRsp - 8);
            return STAT_ADR;
        }
        sim->pc = imm;
        break;
      ...
      case I_RET: /* 9:0 */ {
        long_t valOfPc = 0;
        long_t valOfRsp = get_reg_val(sim->r, REG_RSP);
        get_long_val(sim->m, valOfRsp, &valOfPc);
        set_reg_val(sim->r, REG_RSP, valOfRsp + 8);
        sim->pc = valOfPc;
        break;
        }
      case I_PUSHQ: /* A:0 regA:F */
        if (NORM_REG(regA) && NONE_REG(regB)) {
            ...
            set_reg_val(sim->r, REG_RSP, valOfRsp - 8);
            //printf("\n\n%x\n\n", valOfRsp - 8);
            if (!set_long_val(sim->m, valOfRsp - 8, valA)) {
                err_print("PC = 0x%lx, Invalid stack address 0x%016lx", sim->pc, valOfRsp - 8);
                return STAT_ADR;
            }           
        }
      ...
      case I_POPQ: /* B:0 regA:F */
    	if (NORM_REG(regA) && NONE_REG(regB)) {
            long_t valA;
            long_t valOfRsp = get_reg_val(sim->r, REG_RSP);
            get_long_val(sim->m, valOfRsp, &valA);
